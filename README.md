...
| File | What it does |
|------|-------------|
| `.claude/skills/copy-segmentation-system.md` | Defines how to split outbound lists into executable copy segments. |
| `.claude/skills/copy-message-architecture.md` | Builds first-touch emails with a constrained 4-block structure. |
| `.claude/skills/copy-subject-opener-engine.md` | Generates subject and opener variants with anti-spam controls. |
| `.claude/skills/copy-cta-friction-design.md` | Selects CTA style by touch stage and buyer readiness. |
| `.claude/skills/copy-follow-up-sequence.md` | Designs touches 2-5 with new value and close logic. |
| `.claude/skills/copy-quality-compliance-review.md` | Runs quality, deliverability, and compliance checks before launch. |
...
1. Clone the repo:
   ```bash
   git clone https://github.com/forma-norden/cold-email-copy-playbook.git
   ```
2. Copy `.claude/skills/` into your project:
   ```bash
   cp -r cold-email-copy-playbook/.claude/skills your-project/.claude/
   ```
3. Load the relevant skill before writing any campaign copy.
